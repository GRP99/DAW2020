...
function classificar(nmr, idF) {
  $.ajax({
    url: "http://localhost:3001/files/classificar/" +
    idF +
    "?token=" +
    token +
    "&class=" +
...
function showFile(id, name, type, autor, desc) {
  ...
  var download = $(
    '<div style="margin: auto; width: 25%; border: 2px solid black; text-align: center;"><a href="http://localhost:3001/files/download/' +
      autor +
      "/" +
      id +
      "?token=" +
...
function validate() {
  ...
  $.get(
    "http://localhost:3001/users/" + mail + "?token=" + token,
    function (data) {
      if (data) {
...
function openUploadModal(user) {
  var file = $(
    '<form class="w3-container" onSubmit="return confirm(&quot;Do you want to submit?&quot;)" action="http://localhost:3001/files?token=' +
      token +
      ' "method="POST" enctype="multipart/form-data" id="myForm"><label class="w3-text-blue-grey"><b>Select file</b></label><!-- #addeds--><input class="w3-input w3-border w3-light-grey" type="file" name="myFile" />' +
...
function openWarningModal(user) {
  var file = $(
    '<form class="w3-container" onSubmit="return confirm(&quot;Do you want to submit?&quot;)" action="http://localhost:3001/news?token=' +
      token +
...
function deleteFile(id) {
  ...
  if (choice) {
    $.ajax({
      ...
      type: "DELETE",
      sucess: function(){document.getElementById(id).remove()}
    });
  }
  else false;
}
